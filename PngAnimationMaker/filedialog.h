...
#include <string>     // Required for: strcpy()
#include <rlgl.h>

void GuiDrawCircleSector(Rectangle bounds, int startAngle, int endAngle, int radius, Color color)
{
    // Convert angles to radians
    float startAngleRad = (float)startAngle * DEG2RAD;
    float endAngleRad = (float)endAngle * DEG2RAD;

    // Compute circle center
    Vector2 center = { bounds.x + radius, bounds.y + radius };

    // Compute number of segments
    float segmentAngle = fabsf(endAngleRad - startAngleRad);
    int segmentCount = (int)ceilf(segmentAngle / (PI / 4)) + 1;

    // Allocate vertex array
    Vector2* vertices = (Vector2*)malloc(segmentCount * sizeof(Vector2));

    // Fill vertex array
    for (int i = 0; i < segmentCount; i++)
    {
        float angle = startAngleRad + segmentAngle * ((float)i / (float)(segmentCount - 1));
...
GuiFileDialogState InitGuiFileDialog(const char* initPath)
{
    ...
    // Init window data
    state.windowBounds = { (float)GetScreenWidth() / 2 - 440 / 2, (float)GetScreenHeight() / 2 - 310 / 2, 440, 310 };
...
void GuiFileDialog(GuiFileDialogState* state)
{
    if (state->windowActive)
    {
        ...
        //----------------------------------------------------------------------------------------
        state->windowActive = !GuiWindowBoxModified(state->windowBounds, "#198# Select File");

        // Draw previous directory button + logic
        if (GuiButtonRounded({ state->windowBounds.x + state->windowBounds.width - 48, state->windowBounds.y + 24 + 12, 40, 24 }, "< .."))
        {
        ...
        if (GuiTextBoxFix({ state->windowBounds.x + 8, state->windowBounds.y + 24 + 12, state->windowBounds.width - 48 - 16, 24 }, state->dirPathText, 1024, state->dirPathEditMode))
        {
...
# else
        state->filesListActive = GuiListViewEx({ state->windowBounds.x + 8, state->windowBounds.y + 48 + 20, state->windowBounds.width - 16, state->windowBounds.height - 60 - 16 - 68 }, (const char**)dirFilesIcon, state->dirFiles.count, & state->itemFocused, & state->filesListScrollIndex, state->filesListActive);
# endif
        ...
        if (GuiTextBoxFix({ state->windowBounds.x + 72, state->windowBounds.y + state->windowBounds.height - 68, state->windowBounds.width - 184, 24 }, state->fileNameText, 128, state->fileNameEditMode))
        {
        ...
        state->fileTypeActive = GuiComboBox({ state->windowBounds.x + 72, state->windowBounds.y + state->windowBounds.height - 24 - 12, state->windowBounds.width - 184, 24 }, "All files", state->fileTypeActive);

        state->SelectFilePressed = GuiButtonRounded({ state->windowBounds.x + state->windowBounds.width - 96 - 8, state->windowBounds.y + state->windowBounds.height - 68, 96, 24 }, "Select");

        if (GuiButtonRounded({ state->windowBounds.x + state->windowBounds.width - 96 - 8, state->windowBounds.y + state->windowBounds.height - 24 - 12, 96, 24 }, "Cancel")) state->windowActive = false;
        //--------------------------------------------------------------------------------------
